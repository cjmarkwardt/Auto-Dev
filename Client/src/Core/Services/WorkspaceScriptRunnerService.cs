    ...
    ILogger<WorkspaceScriptRunnerService> logger) : IWorkspaceScriptRunner
{
    ...
    {
        if (!_runCancellations.TryGetValue(scriptId, out var cts))
        {
            return false;
        }
    ...
    private async Task RunAndTrackAsync(ScriptRef script, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var fullPath = Path.Combine(workspacePath, script.Path);
            ...
            var wasStopped = _userStopped.ContainsKey(script.Path);
