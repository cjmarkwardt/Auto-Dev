...
public sealed partial class ScriptTabViewModel : ViewModelBase, IDisposable
{
    ...
    {
        var scripts = await _metadataStore.LoadRunScriptRefsAsync(_workspacePath);
    ...
    private ScriptEntry GetOrCreateEntry(string id, string name)
    {
        ...
        var entry = new ScriptEntry(id, name);
        Entries.Add(entry);
        return entry;
    }
    ...
    {
        var runs = await _metadataStore.LoadScriptRunsAsync(_workspacePath, scriptId);
    ...
    {
        var entry = GetOrCreateEntry(script.Path, script.Name);
    ...
    {
        var entry = Entries.FirstOrDefault(e => e.Id == record.FilePath);
