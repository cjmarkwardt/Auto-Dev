...
The one place the flat singleton list doesn't apply is **per-workspace state**, which by
definition needs a fresh instance every time a workspace is opened. That's `IWorkspaceFactory`
(`ViewModels/IWorkspaceFactory.cs`):

```csharp
public sealed class WorkspaceFactory(/* ~16 shared singletons */) : IWorkspaceFactory
{
...
An AutoDev window only ever has a single workspace open at a time - opening another one replaces
it (see below), and opening a workspace concurrently in a second window means launching a whole
new AutoDev process (`INewInstanceService`, wired to the title bar's own icon button).

`MainShellViewModel` owns a `HeaderViewModel` and a single nullable `Workspace` property. It
subscribes to `Header.WorkspaceOpened`: if the opened path matches the already-open workspace,
this is a no-op; otherwise the previous workspace (if any) is disposed, `IWorkspaceFactory.Create`
builds the new one, `Workspace` is set to it, and `workspace.InitializeAsync()` runs.

The window always starts with `Workspace` null - nothing is restored from a previous run (see
`MainShellViewModel.InitializeAsync`, which only refreshes the account/recent-workspaces state).
While `Workspace` is null, the content area shows the empty state instead: **Open**/**Clone**
buttons and the recent-workspaces list, both driven by `HeaderViewModel` (see `MainShellView.axaml`).

**`WorkspaceViewModel`** owns four sub-view-models - `Version` (git sidebar), `Files` (file
