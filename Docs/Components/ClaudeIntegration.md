...
(including any AI-assisted conflict-resolution turn) and every interjection (see
...
(`sessionKey` is the branch id, or `null` while detached at a tag/commit). Each session key has its
...
e.g. the AI-assisted conflict-resolution loop) reuses the exact same `PauseAsync`/`ResumeAsync`
machinery as a genuine request, just with no request card
...
completion instead of returning immediately - used by the AI-assisted conflict-resolution loop.
...
(mirrored from `FilesSectionViewModel.HasRunningScripts`, itself driven by the script runner) both
gate `CanSend()`. The rationale is symmetric with why
