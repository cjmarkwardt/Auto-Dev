...
extension) - see `UpdateRenderedContent()`. For those files, `src/Core/Services/MermaidMarkdownProcessor`
...
- `src/Core/Services/MermaidRenderer` wraps **Mermaider** (a pure-.NET Mermaid-to-SVG engine - no
...
The same `MermaidMarkdownProcessor` is reused by the Generate tab to render diagrams inside a
Claude reply.
