...
- **`CloneAsync`** - prompts for a URL and a destination parent folder, derives the repo name from
  ...
  success, it opens the result exactly like any other folder - an empty cloned remote gets silently
  `git init`-ed into AutoDev's own branch convention the same as any other empty folder would.
...
run - see `MainShellViewModel`'s `WorkspaceOpened` handler. Opening a second workspace concurrently
means launching a whole new AutoDev process
...
`src/Core/Services/FileTreeService` is a plain filesystem wrapper (`GetChildren`,
...
- **`.fileignore` present** - it replaces `.gitignore` for this purpose entirely (a file `.gitignore`
  ...
  git status/staging regardless, since none of this touches git). `src/Core/Services/FileIgnoreMatcher`
...
The Edit tab refuses to auto-load a file that's either over 100 KB or
...
- **Content** - a plain `File.ReadAllLines` scan per non-ignored, non-binary file (binary check via
  `src/Core/Services/BinaryFileDetector`), case-insensitive substring match, capped at 200 results, run
