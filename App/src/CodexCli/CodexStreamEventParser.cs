...
internal static class CodexStreamEventParser
{
    ...
    {
        if (!root.TryGetProperty("item", out JsonElement item) || !item.TryGetProperty("type", out JsonElement itemTypeEl))
        {
            return null;
        }
    ...
    {
        using JsonDocument inputDoc = JsonDocument.Parse(JsonSerializer.Serialize(new Dictionary<string, string> { [inputProperty] = inputValue }));
