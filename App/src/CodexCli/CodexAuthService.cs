...
public sealed class CodexAuthService(ILogger<CodexAuthService> logger) : IAiAuthService
{
    ...
    public async Task<AiAuthStatus> GetStatusAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            string authFilePath = Path.Combine(CodexHomeDirectory(), "auth.json");
            if (!File.Exists(authFilePath))
            {
                return AiAuthStatus.NotLoggedIn;
            }

            await using FileStream stream = File.OpenRead(authFilePath);
            using JsonDocument document = await JsonDocument.ParseAsync(stream, cancellationToken: cancellationToken);
            JsonElement root = document.RootElement;

            if (!root.TryGetProperty("tokens", out JsonElement tokens) || !tokens.TryGetProperty("id_token", out JsonElement idTokenEl))
            {
                return AiAuthStatus.NotLoggedIn;
            }
            ...
            string? email = payload.TryGetProperty("email", out JsonElement emailEl) ? emailEl.GetString() : null;
            string? planType = null;
            if (payload.TryGetProperty("https://api.openai.com/auth", out JsonElement authClaims) &&
                authClaims.TryGetProperty("chatgpt_plan_type", out JsonElement planEl))
            {
