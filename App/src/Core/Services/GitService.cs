...
public sealed class GitService : IGitService
{
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["rev-parse", "--is-inside-work-tree"], cancellationToken);
    ...
    {
        BufferedCommandResult result = await RunAsync(parentDirectory, ["clone", url, destinationName], cancellationToken);
        if (result.ExitCode == 0)
        {
            return new GitCloneResult(true, null);
        }

        string errorMessage = result.StandardError.Trim();
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["symbolic-ref", "--short", "-q", "HEAD"], cancellationToken);
        return result.ExitCode == 0 ? result.StandardOutput.Trim() : null;
    }

    public async Task<string?> GetExactTagAsync(string workspacePath, CancellationToken cancellationToken = default)
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["describe", "--tags", "--exact-match", "HEAD"], cancellationToken);
        return result.ExitCode == 0 ? result.StandardOutput.Trim() : null;
    }
    ...
    public async Task<IReadOnlySet<string>> GetIgnoredPathsAsync(string workspacePath, IReadOnlyList<string> paths, CancellationToken cancellationToken = default)
    {
        ...
        BufferedCommandResult result = await RunAsync(workspacePath, ["check-ignore", "--stdin", "-z"], cancellationToken, PipeSource.FromString(stdin));
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["status", "--porcelain", "--ignored", "--", path], cancellationToken);
        IReadOnlyList<string> lines = SplitLines(result.StandardOutput);
        if (lines.Count == 0)
        {
        ...
        BufferedCommandResult tracked = await RunAsync(workspacePath, ["ls-files", "--", path], cancellationToken);
    ...
    public async Task<IReadOnlyDictionary<string, GitFileStatus>> GetStatusesAsync(string workspacePath, IReadOnlyList<string> paths, CancellationToken cancellationToken = default)
    {
        ...
        BufferedCommandResult result = await RunAsync(workspacePath, ["status", "--porcelain", "--ignored", "-z", "--untracked-files=all"], cancellationToken);
        ...
        for (int i = 0; i < fields.Length; i++)
        {
            ...
            char indexStatus = field[0];
            char worktreeStatus = field[1];
        ...
        {
            string relative = RelativeTo(workspacePath, queryPath);

            GitFileStatus? resolved = null;
            bool sawIgnored = false;
            foreach ((string? entryPath, char indexStatus, char worktreeStatus) in entries)
            {
        ...
        if (needsTrackedCheck.Count > 0)
        {
            ...
            BufferedCommandResult tracked = await RunAsync(workspacePath, lsFilesArgs, cancellationToken);
            IReadOnlyList<string> trackedPaths = SplitLines(tracked.StandardOutput);

            foreach (string queryPath in needsTrackedCheck)
            {
                string relative = RelativeTo(workspacePath, queryPath);
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["tag", "--list", $"{prefix}*"], cancellationToken);
    ...
    {
        BufferedCommandResult local = await RunAsync(workspacePath, ["branch", "--list", $"{prefix}*", "--format=%(refname:short)"], cancellationToken);
        BufferedCommandResult remote = await RunAsync(workspacePath, ["branch", "-r", "--list", $"origin/{prefix}*", "--format=%(refname:short)"], cancellationToken);
    ...
    {
        BufferedCommandResult verify = await RunAsync(workspacePath, ["show-ref", "--verify", "--quiet", $"refs/heads/{branchName}"], cancellationToken);
    ...
    public async Task<bool> DeleteRemoteBranchAsync(string workspacePath, string branchName, CancellationToken cancellationToken = default)
    {
        ...
        BufferedCommandResult result = await RunAsync(workspacePath, ["push", "origin", "--delete", branchName], cancellationToken);
    ...
    public async Task<bool> DeleteRemoteTagAsync(string workspacePath, string tagName, CancellationToken cancellationToken = default)
    {
        ...
        BufferedCommandResult result = await RunAsync(workspacePath, ["push", "origin", "--delete", $"refs/tags/{tagName}"], cancellationToken);
        return result.ExitCode == 0;
    }
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["rebase", ontoRef], cancellationToken);
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["rebase", "--continue"], cancellationToken);
        return await ClassifyOperationResultAsync(workspacePath, result, cancellationToken);
    }
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["diff", "--name-only", "--diff-filter=U"], cancellationToken);
        return SplitLines(result.StandardOutput);
    }

    public async Task<string?> GetRemoteUrlAsync(string workspacePath, CancellationToken cancellationToken = default)
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["remote", "get-url", "origin"], cancellationToken);
        return result.ExitCode == 0 ? result.StandardOutput.Trim() : null;
    }

    public async Task SetRemoteAsync(string workspacePath, string url, CancellationToken cancellationToken = default)
    {
        string? existing = await GetRemoteUrlAsync(workspacePath, cancellationToken);
    ...
    public async Task<bool> FetchAsync(string workspacePath, bool prune = false, CancellationToken cancellationToken = default)
    {
        ...
        BufferedCommandResult result = await RunAsync(workspacePath, args, cancellationToken);
        return result.ExitCode == 0;
    }
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["rev-parse", $"origin/{branchName}"], cancellationToken);
        return result.ExitCode == 0 ? result.StandardOutput.Trim() : null;
    }
    ...
    public async Task<bool> FastForwardPullAsync(string workspacePath, string branchName, CancellationToken cancellationToken = default)
    {
        ...
        BufferedCommandResult result = await RunAsync(workspacePath, ["merge", "--ff-only", $"origin/{branchName}"], cancellationToken);
        return result.ExitCode == 0;
    }
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["merge", "--ff-only", refName], cancellationToken);
        return result.ExitCode == 0;
    }
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["merge-base", refA, refB], cancellationToken);
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["stash", "push", "-u"], cancellationToken);
        return result.ExitCode == 0;
    }

    public async Task<GitOperationOutcome> StashPopAsync(string workspacePath, CancellationToken cancellationToken = default)
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["stash", "pop"], cancellationToken);
        return await ClassifyOperationResultAsync(workspacePath, result, cancellationToken);
    }
    ...
    public async Task<IReadOnlyDictionary<string, IReadOnlyList<GitTag>>> GetTagsByCommitAsync(string workspacePath, CancellationToken cancellationToken = default)
    {
        ...
        Dictionary<string, List<GitTag>> map = new Dictionary<string, List<GitTag>>();
        foreach (string line in SplitLines(result.StandardOutput))
        {
            ...
            string commitHash = parts[1].Length > 0 ? parts[1] : parts[0];
            string name = parts[2];
            ...
            bool isAnnotated = parts[3] == "tag";
            string displayName = isAnnotated && parts[4].Length > 0 ? parts[4] : name;

            if (!map.TryGetValue(commitHash, out List<GitTag>? tags))
            {
                tags = [];
                map[commitHash] = tags;
            }
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["diff-tree", "--no-commit-id", "--name-status", "-r", "--root", commitHash], cancellationToken);
        ...
        foreach (string line in SplitLines(result.StandardOutput))
        {
            ...
            GitChangeStatus status = parts[0][0] switch
            {
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["show", $"{commitHash}:{relativePath}"], cancellationToken);
    ...
    public async Task<IReadOnlyList<GitChange>> GetWorkingTreeChangesAsync(string workspacePath, CancellationToken cancellationToken = default)
    {
        ...
        BufferedCommandResult result = await RunAsync(workspacePath, ["status", "--porcelain", "-z", "--untracked-files=all"], cancellationToken);
        string[] fields = result.StandardOutput.Split('\0', StringSplitOptions.RemoveEmptyEntries);
        ...
        for (int i = 0; i < fields.Length; i++)
        {
            ...
            string path = entry[3..];
            bool isRenameOrCopy = indexStatus is 'R' or 'C' || worktreeStatus is 'R' or 'C';
            if (isRenameOrCopy)
            {
                i++; // the next field is the original path - unused, see doc comment
            }

            GitChangeStatus status = (indexStatus, worktreeStatus) switch
            {
    ...
    {
        BufferedCommandResult result = await RunAsync(workspacePath, ["status", "--porcelain"], cancellationToken);
    ...
    private static async Task<IReadOnlyList<GitCommit>> LogCommitsAsync(string workspacePath, string revisionRange, CancellationToken cancellationToken)
    {
        ...
        {
            string[] parts = line.Split(sep);
            if (parts.Length == 3 && DateTimeOffset.TryParse(parts[1], out DateTimeOffset date))
            {
    ...
    private static async Task<BufferedCommandResult> RunAsync(string workspacePath, IReadOnlyList<string> args, CancellationToken cancellationToken, PipeSource? standardInput = null)
    {
        ...
        Action<string>? sink = GitCommandLogSink.Current;
