...
public sealed class FileIgnoreMatcher
{
    ...
    public bool IsMatch(string relativePath, bool isDirectory)
    {
        ...
        for (int i = 0; i < segments.Length; i++)
        {
            ...
            bool prefixIsDirectory = i < segments.Length - 1 || isDirectory;
    ...
    {
        bool hasLeadingSlash = pattern.StartsWith('/');
        string body = pattern.TrimStart('/');
        bool anchored = hasLeadingSlash || body.Contains('/');

        string translated = TranslateGlob(body);
        string full = anchored ? $"^{translated}$" : $"(^|.*/){translated}$";
