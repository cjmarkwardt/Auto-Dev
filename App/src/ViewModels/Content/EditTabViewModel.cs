...
public sealed partial class EditTabViewModel(IFileTreeService fileTreeService, IExternalOpenService externalOpenService) : ViewModelBase
{
    ...
    {
        if (Uri.TryCreate(url, UriKind.Absolute, out Uri? uri) && uri.Scheme is "http" or "https" or "mailto" or "ftp")
        {
        ...
        string resolved = Path.GetFullPath(Path.Combine(baseDir, Uri.UnescapeDataString(target)));
    ...
    public async Task LoadDiffAsync(string displayTitle, FileDiffContent diff)
    {
        ...
        DiffLines.Clear();
        foreach (DiffLine? line in await Task.Run(() => BuildDiffLines(diff.Before, diff.After)))
        {
            DiffLines.Add(line);
        }
    }
    ...
    {
        DiffPaneModel diff = InlineDiffBuilder.Diff(before ?? "", after ?? "");
