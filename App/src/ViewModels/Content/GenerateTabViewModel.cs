...
public sealed partial class GenerateTabViewModel : ViewModelBase, IAsyncDisposable
{
    ...
    public async Task SwitchSessionAsync(string? sessionKey)
    {
        ...
        foreach (GenerateRequest request in loaded)
        {
            ...
            GenerateRequestViewModel requestVm = GenerateRequestViewModel.FromModel(request);
    ...
    private async Task SendAsync()
    {
        ...
        List<ImageAttachment> images = Attachments.Select(a => new ImageAttachment(a.MediaType, a.Base64Data)).ToList();
        List<ChatFileAttachment> fileAttachments = FileAttachments.ToList();
        Attachments.Clear();
        FileAttachments.Clear();
        InputText = "";
        ...
        string textWithFileReferences = fileAttachments.Count > 0
    ...
    {
        GenerateRequestViewModel request = new GenerateRequestViewModel
        {
    ...
    {
        "Bash" => TryGetString(toolUse.Input, "description", out string? desc) ? desc
            : TryGetString(toolUse.Input, "command", out string? cmd) ? $"Running: {cmd}" : "Running a command",
        "Read" => TryGetString(toolUse.Input, "file_path", out string? readPath) ? $"Reading {Path.GetFileName(readPath)}" : "Reading a file",
        "Edit" => TryGetString(toolUse.Input, "file_path", out string? editPath) ? $"Editing {Path.GetFileName(editPath)}" : "Editing a file",
        "Write" => TryGetString(toolUse.Input, "file_path", out string? writePath) ? $"Writing {Path.GetFileName(writePath)}" : "Writing a file",
        "Grep" => TryGetString(toolUse.Input, "pattern", out string? pattern) ? $"Searching for \"{pattern}\"" : "Searching",
        "Glob" => TryGetString(toolUse.Input, "pattern", out string? globPattern) ? $"Finding files matching {globPattern}" : "Finding files",
        "TodoWrite" => "Updating task list",
        "WebFetch" => TryGetString(toolUse.Input, "url", out string? url) ? $"Fetching {url}" : "Fetching a URL",
        "WebSearch" => TryGetString(toolUse.Input, "query", out string? query) ? $"Searching the web for \"{query}\"" : "Searching the web",
        "McpToolCall" => TryGetString(toolUse.Input, "description", out string? mcpDesc) ? mcpDesc : "Calling an MCP tool",
        _ => $"Using {toolUse.Name}",
    };

    private static bool TryGetString(JsonElement input, string property, out string value)
    {
        if (input.ValueKind == JsonValueKind.Object && input.TryGetProperty(property, out JsonElement prop) && prop.ValueKind == JsonValueKind.String)
        {
