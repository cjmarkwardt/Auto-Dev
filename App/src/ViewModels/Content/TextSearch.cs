...
internal static class TextSearch
{
    public static List<int> FindAllMatches(string content, string pattern, bool matchCase, bool matchWholeWord)
    {
        ...
        {
            int found = content.IndexOf(pattern, index, comparison);
            if (found < 0)
            {
                break;
            }
    ...
    {
        bool leftBoundary = start == 0 || !IsWordChar(content[start - 1]);
        int end = start + length;
        bool rightBoundary = end == content.Length || !IsWordChar(content[end]);
