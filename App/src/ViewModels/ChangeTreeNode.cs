...
public sealed partial class ChangeTreeNode : ObservableObject
{
    ...
    {
        IsExpanded = false;
        foreach (ChangeTreeNode child in Children)
        {
            child.CollapseAll();
        }
    }
    ...
    {
        ChangeTreeNode root = new ChangeTreeNode { Name = "", IsDirectory = true };
        foreach (GitChange change in changes)
        {
            string[] parts = change.Path.Split('/', StringSplitOptions.RemoveEmptyEntries);
            ChangeTreeNode current = root;
            for (int i = 0; i < parts.Length; i++)
            {
                bool isLast = i == parts.Length - 1;
                ChangeTreeNode? existing = current.Children.FirstOrDefault(c => c.Name == parts[i] && c.IsDirectory == !isLast);
                if (existing is null)
                {
    ...
    private static void SortRecursive(ChangeTreeNode node)
    {
        ...
        foreach (ChangeTreeNode child in node.Children)
        {
            SortRecursive(child);
        }
    }
